...
class SGD(Optimizer):
    ...
    def checkpoint(self, folder):
        with open(os.path.join(folder, "_SGD.json"), 'w') as f:
            json.dump(self.__dict__, f, cls=NumpyEncoder)

    def reload(self, folder):
        with open(os.path.join(folder, "_SGD.json"), 'r') as f:
...
class Adam(Optimizer):
    ...
    def checkpoint(self, folder):
        with open(os.path.join(folder, "_Adam.json"), 'w') as f:
            json.dump(self.__dict__, f, cls=NumpyEncoder)

    def reload(self, folder):
        with open(os.path.join(folder, "_Adam.json"), 'r') as f:
            self.__dict__ = json.load(f, cls=NumpyDecoder)
