...
class Niche:
    ...
    def rollout_eval_batch(self, thetas, batch_size, random_state, eval=False):
        import numpy as np
        returns = np.zeros(batch_size)
        lengths = np.zeros(batch_size, dtype='int')
            ...
            if render_mode:
                if render_mode == 'rgb_array':
                    if len(frames) > 0:
                        ...
                        filename = os.path.join(save_dir, f"opt_{self.env_evo_params.get_name()}_{temp}_reward_{total_reward}.gif")
