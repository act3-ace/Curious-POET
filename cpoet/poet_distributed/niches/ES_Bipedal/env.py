...
class BipedalWalkerCustom(gym.Env):
    ...
    self.env_seed: int
        Seed
    self.viewer: gym.Viewer
        Viewer object for scene rendering
    ...
    self.prev_shaping: float
        If not None, subtract previous step reward off of current step reward each step resulting in reward that shows (pos or neg) improvement each step
    self.fd_polygon: Box2D.b2.fixtureDef
        Abstract fixture, I think this corresponds to the terrain the walker uses in the case where we're not using the cppn to create the terrain.
    self.fd_edge: Box2D.b2.fixtureDef
        Abstract fixture, I think this corresponds to the terrain the walker uses in the case where we are using the cppn to create the terrain.
    ...
    -------
    __repr__()
        Called when BipedalWalkerCustom is cast to string
    __init__(env_config)
        Sets up environment with initial config
    set_env_config(env_config)
        Sets env_config
    augment(params)
        Sets params
    _set_terrain_number()
        Determines which terrain features exist and sets the total terrain number
    save_env_def(filename)
        Save config as json
    seed(seed=None)
        Calls _seed
    _seed(seed=None)
    ...
    _destroy()
        Resets all env variables to empty
    _generate_terrain()
        Generates new terrain for the env. Called by reset.
    _generate_clouds()
        Generates clouds for the renderer :)
    reset()
        Calls _reset
    _reset():
        Resets simulation
    step(action)
        Calls _step
    _step(action)
        Performs a single env step based on action. Returns state, reward, done, info.
    render(*args, **kwargs)
        Calls _render
    _render(mode='human', close=False)
        Render frame from current env state.
    '''
