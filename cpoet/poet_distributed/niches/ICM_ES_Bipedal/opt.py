...
class OptimizationController:
    ...
    def combine_rewards(self, extrinsic_rewards, intrinsic_rewards, gamma:float = 1.0):
        """
        combines the extrinsic and intrinsic rewards - element wise
        Also, the intrinsic rewards are scaled by gamma
        extrinsic reward is a list of POResult objects.  The 'returns' array is modified in place with
        the addition of scaled intrinisc reward.  The returns object is of length 2 due to mirror sampling.

        Parameters:
        extrinsic_rewards: [POResult]  # list of batches_per_chunk length. 
            Returns member is a list of the cumulative reward returned by the environment (one scalar per trajectory)
        intrinsic_rewards: np array
            the inferred reward returned from the ICM trainer
        gamma: float
            Scaling value for intrinsic reward
        """
        assert len(extrinsic_rewards) == intrinsic_rewards.shape[0] ,\
            "intrinisc reward must have same shape as extrinsic reward for element wise combination"
        for i in range(len(extrinsic_rewards)):
            for j in range(2):
                extrinsic_rewards[i].returns[0][j] += gamma * intrinsic_rewards[i][j] 
        return extrinsic_rewards

    def combine_steps(self, step_results, theta, step_t_start, icm, optim_id = "Unnamed", propose_with_adam=True, decay_noise=True, propose_only=False, combine_rewards=True):
        #po_noise_inds = np.concatenate([r.noise_inds for r in step_results])
        po_returns = np.concatenate([r.returns for r in step_results]) # want returns here to only include environment rewards
        po_lengths = np.concatenate([r.lengths for r in step_results])
        rollouts = [r.rollouts for r in step_results]

        inference_start_time = time.time()

        if combine_rewards:
            with open(os.path.join(icm['icm_path'], 'icm', 'hostinfo.json'), 'r') as f:
                address = json.load(f)
            #######################
            net_start = time.time()
            msg = {
                'rollouts': rollouts,
                'iteration': icm['iteration'],
                'optim_id': optim_id,
                'shape': len(step_results),
                'extrinsic_rewards': po_returns,
                'episode_lengths': po_lengths,
            }

            pickle_time = time.time()
            pickled_msg = pickle.dumps(msg)
            logger.info(f"pickle time: {time.time()-pickle_time}")

            logger.info(f"sending rollouts to icm server...")
            with Req0(dial=address) as req:
                tx_time = time.time()
                req.send(pickled_msg)
                logger.info(f"tx_time {time.time()-tx_time}")

                rx_time = time.time()
                intrinsic_rewards = pickle.loads(req.recv())
                if icm['iteration'] < icm['icm_stable_iteration']:
                    intrinsic_rewards = np.zeros((len(step_results), 2))
                logger.info(f"rx_time {time.time()-rx_time}")

            inference_time = time.time() - inference_start_time
            logger.info(f"inference time: {inference_time}")

            step_results = self.combine_rewards(step_results, intrinsic_rewards, icm['icm_training_gamma'])
        ...
        #### This needs to be placed in the niche
    ...
    def compute_grads(self, step_results, theta):
        ...
        noise_inds, returns, _, _ = self.collect_po_results(step_results)
    ...
    def collect_po_results(self, po_results):
        ...
        rollouts: list of tuples
            trajectory of (state, action, done) tuples
        '''
        ...
        rollouts = [r.rollouts for r in po_results]
        return noise_inds, returns, lengths, rollouts
