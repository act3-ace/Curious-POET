...
from .opt import OptimizationController
...
class Niche:
    ...
    def class_setup(args):
        if args.niche_params["use_icm"]:
            Niche.icm = {
            'use_icm':True, 
            'icm_path':args.log_file, 
            'iteration':0, 
            'combine_rewards':False,
            'icm_stable_iteration':args.niche_params["icm_stable_iteration"],
            'icm_training_gamma':args.niche_params["icm_training_gamma"],
            }

            os.makedirs(os.path.join(args.log_file, 'icm/rollouts'), exist_ok=True)
            os.makedirs(os.path.join(args.log_file, 'icm/checkpoints'), exist_ok=True)
            os.makedirs(os.path.join(args.log_file, 'icm/icm_stats'), exist_ok=True)
        pass
    ...
    def __init__(self, env_evo_params, seed, init='random', stochastic=False, args=None):
        ...
        self.opt_control = OptimizationController(args, self.model)
        #logger.info('Created OptimizationController ' + str(self.opt_control) )
        self.init = init
    ...
    def __setstate__(self, state):
        ...
        self.opt_control = OptimizationController(self.args, self.model)
    ...
    def _rollout_opt_batch(self, thetas, batch_size, random_state, eval=False):  
        ...
        for i, theta in enumerate(thetas):
            returns[i], lengths[i], rollout = self._rollout(
                theta, random_state=random_state, eval=eval)
            
        return returns, lengths, rollout
    ...
    def rollout_eval_batch(self, thetas, batch_size, random_state, return_rollouts=False):
        ...
        for i, theta in enumerate(thetas):
            returns[i], lengths[i], rollouts = self._rollout(
                theta, random_state=random_state, eval=True)
        if return_rollouts:
            return returns, lengths, rollouts
    ...
    def combine_steps(self, step_results, theta, step_t_start, 
                      propose_with_adam=True, decay_noise=True, 
                      propose_only=False):

        return(self.opt_control.combine_steps(step_results, 
                    theta, 
                    step_t_start, 
                    Niche.icm, 
                    optim_id=self.env_evo_params.get_name(), 
                    propose_with_adam=True, 
                    decay_noise=True, 
                    propose_only=False))

    def _simulate(self, model, env, random_state, train_mode=False, 
                  render_mode=False, num_episode=5, max_len=-1, 
                  env_config_this_sim=None, env_evo_params=None):
        ...
        rollout: list
            List of (state, action, done) tuples
        '''
        ...
        for _ in range(num_episode):
            ...
            for t in range(max_episode_length):
                ...
                total_reward += reward
                done_np = np.expand_dims(np.array(done, dtype = np.float32), axis = 0)
                rollout.append(np.concatenate( (obs.astype('float32'), action.astype('float32'), done_np) ))
                if done:
                    break
            ...
            if render_mode:
                if render_mode == 'rgb_array':
                    if len(frames) > 0:
                        ...
                        if env_evo_params.__doc__ is None:
                            ...
                            save_dir = env_evo_params.__doc__[env_evo_params.__doc__.find('!')+1:]
                            env_num = env_evo_params.__doc__[:env_evo_params.__doc__.find('!')]
                            filename = os.path.join(save_dir, f"envNum_{env_num}_opt_{self.env_evo_params.get_name()}_{temp}.gif")
        ...
        return reward_list, t_list, np.stack(rollout)