...
def main(args):
    ...
    # with open(args.stateactions_stats_pickle_folder, 'rb') as f:
    #     state_action_list = pickle.load(f)
    
    print(f"loading stateaction sets...")
    state_action_list={}
    for fn in os.listdir(args.stateactions_stats_pickle_folder):
        if '.npy' in fn:
            _fn = os.path.join(args.stateactions_stats_pickle_folder, fn)
            print(f"loading {_fn}")
            state_actions=(np.load(_fn))
            state_action_list['']
    print(f"Done")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--stateactions_stats_pickle_folder',
        type=str,
