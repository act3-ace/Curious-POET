...
import pygraphviz as pgv  #apt-get install graphviz libgraphviz-dev
import numpy as np
import pydot
import glob
import json 
...
def main():
    ...
    env_counter = 0
    for it in it_vec: 
        parents = [x['parent'] for x in tuple_data if x['iteration']==it]
        children = [x['child'] for x in tuple_data if x['iteration']==it]
        envs = set(parents + children)
        new_env_ids = [x for x in envs if x not in env_to_id.keys()] 
        for v in new_env_ids: 
            env_to_id[v] = env_counter 
            env_counter += 1


    for i, it in enumerate(it_vec): 
        if i == 0:
            phylo_tuples = [(env_to_id[x['parent']], env_to_id[x['child']]) for x in tuple_data if (x['link_type']=='env_evolution' and x['iteration']<=it)]
        else:
            phylo_tuples = phylo_tuples + [(env_to_id[x['parent']], env_to_id[x['child']]) for x in tuple_data if (x['link_type']=='env_evolution' and x['iteration']==it)]
        transfers_it = [(env_to_id[x['parent']], env_to_id[x['child']]) for x in tuple_data if (x['link_type']=='agent_transfer' and x['iteration']==it)]
        ...
        if i % 10 == 0: 
            ...
            tree.graph_attr.update(dpi=300, size="3,4\!")
        ...
        if i > 0:
            longest_phylo_path[i] = max((dict(nx.shortest_path_length(tree))[env_to_id[tuple_data[0]['parent']]]).values())
        else:
            longest_phylo_path[i]=0
