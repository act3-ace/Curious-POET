...
def parse_args():
    ...
    parser.add_argument("--output_figure_filename", type=str, required=False, default='cross_eval_confusion_dict_new.png')
...
def plot_confusion(ce_data, args):
    ...
    for i in range(len(args.run_names)):
        for j in range(len(args.run_names)):
            ...
            text = axs[1].text(j, i, f"{env_perf[i, j]:0.2f}",      ha="center", va="center", color="k" if env_perf[i, j]>0.2 else "y")    
    ...
    agg_means_env = np.zeros((size, size))
    agg_stds_env = np.zeros((size, size))
            ...
            for agent_idx, agent in enumerate(get_run_names(args)):  # raster over whole matrix, accum_agentulating matching np arrays
                for env_idx, env in enumerate(get_run_names(args)):
                    if gammas[agent_idx] == agg_agent and gammas[env_idx] == agg_env:   #include these eval results
                        ...
                        accum_env.append(env_perf[agent_idx, env_idx])
            # print('next')
            # for i in accum:
            #     print(i.mean(), i.std())
            agg_means_agent[agg_agent_idx, agg_env_idx] = np.mean(accum_agent)
            agg_stds_agent[agg_agent_idx, agg_env_idx] = np.std(accum_agent)
            agg_means_env[agg_agent_idx, agg_env_idx] = np.mean(accum_env)
            agg_stds_env[agg_agent_idx, agg_env_idx] = np.std(accum_env)

    im0 = axs[0][0].imshow(agg_means_agent, cmap='plasma')
    im1 = axs[1][0].imshow(agg_stds_agent, cmap='plasma')
    im2 = axs[0][1].imshow(agg_means_env, cmap='plasma')
    im3 = axs[1][1].imshow(agg_stds_env, cmap='plasma')
    ...
    axs[0][0].set_xticks(np.arange(len(agg_gammas)), labels=agg_gammas)
    axs[1][0].set_xticks(np.arange(len(agg_gammas)), labels=agg_gammas)
    axs[0][0].set_yticks(np.arange(len(agg_gammas)), labels=agg_gammas)
    axs[1][0].set_yticks(np.arange(len(agg_gammas)), labels=agg_gammas)
    axs[0][1].set_xticks(np.arange(len(agg_gammas)), labels=agg_gammas)
    axs[1][1].set_xticks(np.arange(len(agg_gammas)), labels=agg_gammas)
    axs[0][1].set_yticks(np.arange(len(agg_gammas)), labels=agg_gammas)
    axs[1][1].set_yticks(np.arange(len(agg_gammas)), labels=agg_gammas)
    # plt.setp(axs[0].get_xticklabels(), rotation=45, ha="right", rotation_mode="anchor")
    
    axs[0][0].set_ylabel(r'$\zeta_{agents}$')
    axs[1][0].set_ylabel(r'$\zeta_{agents}$')
    axs[0][1].set_ylabel(r'$\zeta_{agents}$')
    axs[1][1].set_ylabel(r'$\zeta_{agents}$')

    axs[0][0].xaxis.set_tick_params(labelbottom=False); axs[0][0].set_xticks([])
    axs[1][0].set_xlabel(r'$\zeta_{environments}$')
    axs[0][1].xaxis.set_tick_params(labelbottom=False); axs[0][1].set_xticks([])
    axs[1][1].set_xlabel(r'$\zeta_{environments}$')

    axs[0][0].set_title(f"agent performance (means)")
    axs[1][0].set_title(f"agent performance (std)")
    axs[0][1].set_title(f"env performance (means)")
    axs[1][1].set_title(f"env performance (std)")

    # Loop over data dimensions and create text annotations.
    for i in range(size):
        for j in range(size):
            axs[0][0].text(j, i, f"{agg_means_agent[i, j]:0.2f}",  ha="center", va="center",
                 color="y" if agg_means_agent[i, j] <0.25 else "k")
            axs[1][0].text(j, i, f"{agg_stds_agent[i, j]:0.2f}", ha="center", va="center", 
                color="y" if agg_stds_agent[i, j] <0.15 else "k")
            axs[0][1].text(j, i, f"{agg_means_env[i, j]:0.2f}",  ha="center", va="center",
                 color="y" if agg_means_env[i, j] <0.25 else "k")
            axs[1][1].text(j, i, f"{agg_stds_env[i, j]:0.2f}", ha="center", va="center", 
                color="y" if agg_stds_env[i, j] <0.15 else "k")
    ...
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(im2, cax=cax, orientation='vertical', shrink=0.755)
    divider = make_axes_locatable(axs[1][1])
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(im3, cax=cax, orientation='vertical', shrink=0.755)  
